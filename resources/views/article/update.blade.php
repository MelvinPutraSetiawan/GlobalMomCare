...
<div class="container p-8">
    ...
    <form method="POST" action="{{ route('articles.update', $article->id) }}" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="col-span-2">
                <div class="mb-6">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" class="w-full p-3 border rounded-lg shadow-sm" placeholder="Enter the article title" required>
                </div>

                <div class="pt-4 h-auto">
                    <label for="content" class="block text-lg font-medium text-gray-700 mb-2">Content</label>
                    <textarea name="content" id="content" rows="8" class="w-full p-3 border rounded-lg shadow-sm" placeholder="Write your article here..." required>{{ old('content', $article->content) }}</textarea>
                </div>
            </div>
        </div>
