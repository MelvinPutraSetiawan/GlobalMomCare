...
<section class="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
  <form method="POST" action="<?php echo e(route('orders.process.payment', $orders->id)); ?>" class="mx-auto max-w-screen-xl px-4 2xl:px-0">
    <?php echo csrf_field(); ?>
            ...
            <?php if($orders->orderDetails->isNotEmpty()): ?>
                <?php $__currentLoopData = $orders->orderDetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
                        ...
                        <a href="<?php echo e(route('products.show', $order->product->id)); ?>" class="shrink-0 md:order-1">
                            <?php if($order->product->pictures->isNotEmpty()): ?>
                                <img src="data:image/jpeg;base64,<?php echo e(base64_encode($picture->pictureLink)); ?>" alt="<?php echo e($order->product->name); ?>" class="w-40 h-32 object-cover rounded">
                            <?php else: ?>
                        ...
                        <div class="flex items-center justify-between md:order-3 md:justify-end">
                            <p class="ml-3 mr-4 w-10 shrink-0 border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 dark:text-white">x<?php echo e($order->quantity); ?></p>
                            <p data-price-display class="text-base font-bold text-gray-900 dark:text-white">Rp <?php echo e(number_format($order->product->price, 0, ',', '.')); ?></p>
                        </div>

                        <div class="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                            <a href="<?php echo e(route('products.show', $order->product->id)); ?>" class="text-base font-medium text-gray-900 hover:underline dark:text-white"><?php echo e($order->product->name); ?></a>
                        </div>
                        </div>
                    </div>
        ...
        <div class="flow-root">
          <div class="-my-3 divide-y divide-gray-200 dark:divide-gray-800">
            <dl class="flex items-center justify-between gap-4 py-3">
              ...
              <dd class="text-base font-medium text-gray-900 dark:text-white">Rp. <?php echo e(number_format($subtotal, 0, ',', '.')); ?></dd>
            </dl>

            <dl class="flex items-center justify-between gap-4 py-3">
              <dt class="text-base font-normal text-gray-500 dark:text-gray-400">Tax (10%)</dt>
              <dd class="text-base font-medium text-gray-900 dark:text-white">Rp. <?php echo e(number_format($tax, 0, ',', '.')); ?></dd>
            </dl>

            <dl class="flex items-center justify-between gap-4 py-3">
              <dt class="text-base font-normal text-gray-500 dark:text-gray-400">Delivery</dt>
              <dd class="text-base font-medium text-gray-900 dark:text-white">Rp. <?php echo e(number_format($delivery, 0, ',', '.')); ?></dd>
            </dl>

            <dl class="flex items-center justify-between gap-4 py-3">
              <dt class="text-base font-bold text-gray-900 dark:text-white">Total</dt>
              <dd class="text-base font-bold text-gray-900 dark:text-white">Rp. <?php echo e(number_format($total, 0, ',', '.')); ?></dd>
            </dl>
          </div>
        </div>
        <div class="space-y-3">
...
<?php echo $__env->make('layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Melvin\Documents\GitHub\GlobalMomsCare\resources\views\order\payment.blade.php ENDPATH**/ ?>